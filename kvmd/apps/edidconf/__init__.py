...
def _smart_open(path: str, mode: str) -> Generator[IO, None, None]:
    ...
    with (os.fdopen(fd, mode, closefd=False) if path == "-" else open(path,
                                                                      mode)) as file:
        yield file
        if "w" in mode:
            file.flush()
...
class _Edid:
    ...
    def __init__(self, path: str) -> None:
        with _smart_open(path, "rb") as file:
            ...
            assert len(
                self.__data) == 256, f"Invalid EDID length: {len(self.__data)}, should be 256 bytes"
            assert self.__data[126] == 1, "Zero extensions number"
            assert (self.__data[128], self.__data[129]) == (
            0x02, 0x03), "Can't find CEA-861"
...
# =====
def main(argv: (
        list[str] | None) = None) -> None:  # pylint: disable=too-many-branches
    ...
    parser.add_argument("-f", "--edid", dest="edid_path",
                        default="/etc/kvmd/tc358743-edid.hex",
                        help="The hex/bin EDID file path", metavar="<file>")
    parser.add_argument("--export-hex",
                        help="Export [--edid] file to the new file as a hex text",
                        metavar="<file>")
    parser.add_argument("--export-bin",
                        help="Export [--edid] file to the new file as a bin data",
                        metavar="<file>")
    parser.add_argument("--import", dest="imp",
                        help="Import the specified bin/hex EDID to the [--edid] file as a hex text",
                        metavar="<file>")
    parser.add_argument("--set-audio", type=valid_bool,
                        help="Enable or disable basic audio",
                        metavar="<yes|no>")
    parser.add_argument("--set-mfc-id",
                        help="Set manufacturer ID (https://uefi.org/pnp_id_list)",
                        metavar="<ABC>")
    ...
    parser.add_argument("--set-monitor-name",
                        help="Set monitor name in DTD/MND (ASCII, max 13 characters)",
                        metavar="<str>")
    parser.add_argument("--clear", action="store_true",
                        help="Clear the EDID in the [--device]")
    parser.add_argument("--apply", action="store_true",
                        help="Apply [--edid] on the [--device]")
    parser.add_argument("--device", dest="device_path",
    ...
    try:
        if options.clear:
            ...
            if options.apply:
                time.sleep(1)
        if options.apply:
            edid_data = edid.get_data()
            lt6911c_edid_prog(edid_data)
            """
            subprocess.run([
                "/usr/bin/v4l2-ctl",
                f"--device={options.device_path}",
